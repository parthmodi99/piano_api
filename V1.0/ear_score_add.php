...
if (isset($_REQUEST['user_id']) && $_REQUEST['user_id'] != '') {
    ...
    if ($check_user->num_rows > 0) {
        ...
        if ($sel_chapter->num_rows > 0) {
            ...
            $add_score = $con->query("INSERT INTO tbl_ear_score(`user_id`,`chapter_id`,`type`,`level`,`correct_answer`,`total_time`,`avg_time`,`best_time`) VALUES('".$_REQUEST['user_id']."','".$_REQUEST['chapter_id']."','".$_REQUEST['type']."','".$_REQUEST['level']."','".$_REQUEST['correct_answer']."','".$_REQUEST['total_time']."','".$_REQUEST['avg_time']."','".$best_time."')");

            if ($add_score) {
                $rec_cnt = $con->query("SELECT * FROM tbl_ear_score  WHERE `user_id`='".$_REQUEST['user_id']."' AND chapter_id='".$_REQUEST['chapter_id']."' AND type='major' AND best_time != 0 AND correct_answer >= '16' GROUP BY level, type");
                $total_rec_cnt = $rec_cnt->num_rows;

                if($total_rec_cnt >= 4 && $_REQUEST['lession_id'] != '' && $_REQUEST['course_id'] != ''){
                ...
                $count = $con->query("SELECT * FROM tbl_ear_score WHERE user_id='".$_REQUEST['user_id']."'");
